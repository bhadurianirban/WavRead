...
public static List<RiffChunk> ParseRiff(ByteBuffer fileBuffer, Class<? extends RiffChunk> parentClass)
	...
	while(fileBuffer.position()<fileBuffer.capacity())
		...
		//Convert tag sequence to String
                ...
                Logger.getLogger(RiffChunk_WAVE.class.getName()).log(Level.INFO, "Looking for class to match tag : ''{0}''", tag);
		...
		try
			...
			if (tag.equalsIgnoreCase("WAVE")) {
				Class<RiffChunk_WAVE> chunkClass = RiffChunk_WAVE.class;
				RiffChunk decoder = (RiffChunk)chunkClass.newInstance();
				decoder.fromData(fileBuffer);
				subChunks.add(decoder);
			}
			else if (tag.equalsIgnoreCase("RIFF")) {
				Class<RiffChunk_RIFF> chunkClass = RiffChunk_RIFF.class;
				RiffChunk decoder = (RiffChunk)chunkClass.newInstance();
				decoder.fromData(fileBuffer);
				subChunks.add(decoder);
			}
			else if (tag.equalsIgnoreCase("JUNK")) {
				Class<RiffChunk_JUNK> chunkClass = RiffChunk_JUNK.class;
				RiffChunk decoder = (RiffChunk)chunkClass.newInstance();
				decoder.fromData(fileBuffer);
				subChunks.add(decoder);
			}
			else if (tag.equalsIgnoreCase("fmt_")) {
				Class<RiffChunk_fmt_> chunkClass = RiffChunk_fmt_.class;
				RiffChunk decoder = (RiffChunk)chunkClass.newInstance();
				decoder.fromData(fileBuffer);
				subChunks.add(decoder);
			}
			else if (tag.equalsIgnoreCase("data")) {
				Class<RiffChunk_data> chunkClass = RiffChunk_data.class;
				RiffChunk decoder = (RiffChunk)chunkClass.newInstance();
				decoder.fromData(fileBuffer);
				subChunks.add(decoder);
			}
